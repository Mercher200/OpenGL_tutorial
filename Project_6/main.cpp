...
int main(void)
{
    ...
    glBufferData(GL_ARRAY_BUFFER, 4 * 2 * sizeof(float), positions, GL_STATIC_DRAW); // заполнение фактического буффера
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glBindBuffer(GL_ARRAY_BUFFER, buffer); // привязка вершинного буффера
        glEnableVertexAttribArray(0); //для обрезки массива
        glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE, sizeof(float) * 2, 0); // привязка буффера

        glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, ibo);  //привязыаем индексный буффер
